	...
	CREATE TABLE IF NOT EXISTS wymagane_czesci (
		ilosc integer NOT NULL CHECK(ilosc > 0),
		sablony_napraw_id integer NOT NULL REFERENCES szablony_napraw ON DELETE CASCADE, --fk
	...
	CREATE ROLE "application" WITH
		...
		NOREPLICATION
		CONNECTION LIMIT -1
		PASSWORD 'db2securepassword'; --change to encrypted one
	...
	GRANT SELECT (stanowiska_id, haslo, login) ON pracownicy TO application;
